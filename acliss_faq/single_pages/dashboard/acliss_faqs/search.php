...
<?php echo Loader::helper('concrete/dashboard')->getDashboardPaneHeaderWrapper(t('FAQ Details'), "", false, false); ?>
	<div class="ccm-pane-options">
		<?php print $ih->button(t('Edit Again'), $this->url('/dashboard/acliss_faqs/add/', 'edit', $aclissfaq->getAclissFaqID()), 'left', 'primary'); ?>
		<?php print $ih->button(t('Delete'), $this->url('/dashboard/acliss_faqs/add/', 'confirm_delete', $aclissfaq->getAclissFaqID()), 'left', 'error'); ?>
		<?php print $ih->button(t('Back to List'), $this->url('/dashboard/acliss_faqs/search/'), 'right', 'primary'); ?>
	</div>
	<div class="ccm-pane-body">
		<dl>
			...
			<dd><?php echo $caclissFaq->getAnswer();?></dd>
		</dl>
	</div>
<?php } else { ?>
<?php echo Loader::helper('concrete/dashboard')->getDashboardPaneHeaderWrapper(t('Search FAQ'), t('Search and Edit FAQ.'), false, false); ?>
	<div class="ccm-pane-options" id="ccm-acliss-faq-pane-options">
		<?php
		Loader::packageElement('acliss_faqs/search_form', 'acliss_faq');
		?>
	</div>

	<?php
	Loader::packageElement(
	   'acliss_faqs/search_results', 'acliss_faq', array(
	    'aclissfaq' => $aclissfaq,
